...
class DiffusALNoUnc(DiffusionStrategy):
...
class DiffusALNoImp(DiffusionStrategy):
    selection_type: SelectionType = SelectionType.ITERATIVE_SEQ
...
class DiffusALNoDiv(DiffusionStrategy):
    selection_type: SelectionType = SelectionType.ITERATIVE_SEQ
    ...
    def initialize_labeled_pool(self,
        ...
        super().initialize_labeled_pool(
            num_train=num_train, seed=seed, num_test=num_test, num_val=num_val)
        if self.cluster_masks is None:
            n_classes = self.train_data.y.unique().numel()
            kmeans = KMeans(n_clusters=n_classes, random_state=seed)
            labels = kmeans.fit(self.datawrapper.diffused_features.x.cpu()).labels_
            self.cluster_masks = []
